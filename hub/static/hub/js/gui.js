...
function ClickedSquare(pageX, pageY) {
	...
	// This is a good way to get a divs element with Django tomfoolery
	...
	var file = Math.floor((pageX-posX) / 50);
	var rank = 7 - Math.floor((pageY-posY) / 50);
...
function PieceIsOnSq(sq, top, left) {
	if( (RanksBrd[sq] == 7 - Math.round(top/50) ) && 
		FilesBrd[sq] == Math.round(left/50) ) {
		return BOOL.TRUE;
	}
	return BOOL.FALSE;
}
...
function AddGUIPiece(sq, pce){
	...
	var pieceFileName = url_src + SideChar[PieceCol[pce]] + PceChar[pce].toUpperCase() + ".png";
